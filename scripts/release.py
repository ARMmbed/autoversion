...
def change_remote(repo, github_token, branch_name):
    ...
    new_origin = repo.create_remote("origin", url=new.format(GITHUB_TOKEN=github_token))
    repo.git.fetch()
    repo.git.checkout(branch_name)
    repo.git.branch("--set-upstream-to", "%s/%s" % (new_origin, branch_name))


def commit_release(repo, branch_name, version):
    logging.info("Committing release...")
    modified_files = [f.a_path for f in repo.index.diff(None)] + repo.untracked_files
    if len(modified_files) == 0:
        logging.info("No commit to perform as no changes were detected")
        return
...
def push_to_github(branch_name, repo, force=False):
    ...
    try:
        if force:
            repo.git.push("-f", "--set-upstream", repo.remotes.origin, branch_name)
        else:
            repo.git.push(
                "--follow-tags", "--set-upstream", repo.remotes.origin, branch_name
            )
        return True
...
def get_new_version():
    version = (
        subprocess.check_output(["python", "setup.py", "--version"]).decode().strip()
    )
    if "dev" in version:
...
def main():
    gh_token = os.getenv(ENVVAR_GITHUB_TOKEN) or os.getenv(ENVVAR_GITHUB_TOKEN2)
    # see:
